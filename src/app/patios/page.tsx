...
function SectionDot({ p, pos, label, scrollTo }: { p: MotionValue<number>; pos: number; label: string; scrollTo: () => void }) {
  ...
  return (
    <div className="flex items-center gap-2 cursor-pointer" onClick={scrollTo}>
      <motion.span style={{ opacity: active }} className="hidden md:block">
        <span style={{ color: "#C9A84C", fontSize: "9px", letterSpacing: "0.2em", textTransform: "uppercase" }}>{label}</span>
      </motion.span>
...
const DOTS = [
  ...
  { pos: 0.54, label: "Features" },
  { pos: 0.70, label: "Elements" },
    ...
    <div className="fixed right-6 top-1/2 -translate-y-1/2 z-40 flex flex-col gap-3 items-end">
      {DOTS.map(({ pos, label }) => (
        <SectionDot key={label} p={p} pos={pos} label={label}
          scrollTo={() => { const el = document.documentElement; window.scrollTo({ top: pos * (el.scrollHeight - el.clientHeight), behavior: "smooth" }); }}
        />
      ))}
    </div>
  );
...
export default function PatiosPage() {
  ...
  return (
    <div style={{ background: "#0D0D0D" }}>
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5, duration: 0.6 }} className="fixed top-6 left-6 z-50">
      ...
      <div ref={containerRef} style={{ height: "700vh" }}>
        <div style={{ position: "sticky", top: 0, height: "100vh", overflow: "hidden" }} className="relative">
          ...
          <BgImage src={IMGS.pavers}  p={scrollYProgress} timing={T.bg3} overlay={0.5} />
          <BgImage src={IMGS.firepit} p={scrollYProgress} timing={T.bg4} overlay={0.5} />
          <BgImage src={IMGS.kitchen} p={scrollYProgress} timing={T.bg5} overlay={0.55} />
          <BgImage src={IMGS.outdoor} p={scrollYProgress} timing={T.bg6} overlay={0.35} />
          <BgImage src={IMGS.evening} p={scrollYProgress} timing={T.bg7} overlay={0.82} />
          ...
          <motion.div style={{ opacity: useTransform(scrollYProgress, [0, 0.06], [1, 0]) }}
    ...
    <motion.div style={{ opacity }} className="absolute inset-0 flex flex-col items-center justify-center z-10">
      <motion.div style={{ y }} className="text-center px-6">
        <motion.div style={{ opacity: subOp, y: subY, border: "1px solid rgba(201,168,76,0.25)", background: "rgba(201,168,76,0.05)" }}
          ...
          <span className="text-xs tracking-[0.3em] uppercase" style={{ color: "#C9A84C", fontSize: "10px" }}>Techo-Bloc & Unilock Certified · Northern Virginia</span>
        </motion.div>
        <h1 style={{ fontFamily: "var(--font-display)", fontSize: "clamp(4rem, 14vw, 12rem)", fontWeight: 300, letterSpacing: "-0.03em", lineHeight: 0.9, color: "#F5F0E8" }}>
          Patios &<br />
          <em style={{ background: "linear-gradient(135deg, #9E7A2E, #E8C96A, #C9A84C)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>Hardscaping</em>
        </h1>
        <motion.p style={{ opacity: subOp, y: subY }} className="text-center mt-8">
          <span style={{ color: "#8A8A8A", fontSize: "clamp(0.9rem, 2vw, 1.1rem)", fontWeight: 300 }}>
            Premium pavers and natural stone for Northern Virginia homes.
          </span>
        </motion.p>
      </motion.div>
    </motion.div>
    ...
    <motion.div style={{ opacity }} className="absolute inset-0 flex flex-col items-center justify-center z-10 px-6">
      <motion.div style={{ y: y1 }} className="text-center">
        ...
        <h2 style={{ fontFamily: "var(--font-display)", fontSize: "clamp(2.8rem, 7vw, 7rem)", fontWeight: 300, color: "#F5F0E8", lineHeight: 1.0, letterSpacing: "-0.02em" }}>
          Your Outdoor
          <br />
          <em style={{ color: "#C9A84C" }}>Living Room.</em>
        </h2>
      </motion.div>
      <motion.div style={{ opacity: op2, y: y2 }} className="text-center mt-8 max-w-xl">
        <p style={{ color: "rgba(245,240,232,0.65)", fontSize: "clamp(1rem, 2vw, 1.15rem)", fontWeight: 300, lineHeight: 1.8 }}>
          A professionally designed patio transforms an unused backyard into a stunning
          extension of your home. We use premium paver systems, natural stone, and
          porcelain tile that look extraordinary and outlast wood decks by decades.
        </p>
      </motion.div>
    </motion.div>
  );
...
const PATIO_MATERIALS = [
  { name: "Techo-Bloc", origin: "Premium Concrete Pavers", props: ["Engineered strength", "Frost-resistant", "100s of design options"] },
  { name: "Unilock",    origin: "Concrete Paver Systems",  props: ["Belgard certified", "EnduraColor technology", "Lifetime guarantee"] },
  { name: "Natural Stone", origin: "Bluestone · Granite · Travertine", props: ["Unique natural beauty", "Timeless appeal", "Premium outdoor tile"] },
];

function MaterialCard({ mat, p, delay }: { mat: typeof PATIO_MATERIALS[0]; p: MotionValue<number>; delay: number }) {
    ...
    <motion.div style={{ opacity }} className="absolute inset-0 z-10 flex items-center px-8 md:px-20 lg:px-32">
      <motion.div style={{ y }} className="max-w-6xl w-full mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {PATIO_MATERIALS.map((mat, i) => <MaterialCard key={mat.name} mat={mat} p={p} delay={i * 0.025} />)}
        </div>
      </motion.div>
    </motion.div>
  );
...
const PATIO_TYPES = [
  "Paver Patios — Techo-Bloc & Unilock",
  "Stamped Concrete",
  "Natural Stone & Bluestone",
  "Porcelain Outdoor Tile",
  "Retaining Walls",
  "Built-In Fire Pit & Fire Tables",
  "Outdoor Kitchen Integration",
  "Drainage & Grading Engineering",
];
  ...
  return (
    <motion.div style={{ opacity }} className="absolute inset-0 z-10 flex items-center justify-end px-8 md:px-20 lg:px-32">
      <motion.div style={{ y }} className="max-w-2xl text-right">
        <span className="text-xs tracking-[0.3em] uppercase block mb-6" style={{ color: "#C9A84C", fontSize: "10px" }}>What We Install</span>
        <h2 style={{ fontFamily: "var(--font-display)", fontSize: "clamp(3rem, 7vw, 7rem)", fontWeight: 300, color: "#F5F0E8", lineHeight: 1.0, letterSpacing: "-0.02em", marginBottom: "2rem" }}>
          Every surface.
          <br />
          <em style={{ color: "#C9A84C" }}>Perfected.</em>
        </h2>
        <div className="space-y-3">
          {PATIO_TYPES.map((spec, i) => <SpecItem key={spec} spec={spec} p={p} i={i} />)}
        </div>
      </motion.div>
    </motion.div>
  );
...
const PATIO_FEATURES = [
  { icon: "◈", title: "Built-In Fire Pits", desc: "Custom gas or wood-burning fire pits integrated seamlessly into your patio design." },
  { icon: "✦", title: "Outdoor Kitchens", desc: "Full outdoor kitchen stations with grill, refrigerator, prep area, and bar seating on your patio." },
  { icon: "◎", title: "Retaining Walls", desc: "Engineered retaining walls in matching paver or natural stone for sloped Northern Virginia lots." },
  { icon: "❖", title: "Landscape Integration", desc: "Built-in planters, garden beds, and lighting designed alongside the patio for a cohesive look." },
  { icon: "⬡", title: "Proper Drainage", desc: "All patios are graded and drained correctly — no pooling, no erosion, no warranty issues." },
  { icon: "◻", title: "LED Lighting", desc: "In-ground LED step lights, path lights, and wall-wash fixtures for evening ambiance." },
];

function FeatureCard({ feat, p, i }: { feat: typeof PATIO_FEATURES[0]; p: MotionValue<number>; i: number }) {
  const delay = i * 0.018;
    ...
    <motion.div style={{ opacity }} className="absolute inset-0 z-10 flex items-center px-8 md:px-16 lg:px-24">
      <div className="max-w-6xl w-full mx-auto">
        <motion.div style={{ y: titleY }}>
          <span className="text-xs tracking-[0.3em] uppercase block mb-4" style={{ color: "#C9A84C", fontSize: "10px" }}>Patio Add-Ons</span>
          <h2 style={{ fontFamily: "var(--font-display)", fontSize: "clamp(2.5rem, 6vw, 6rem)", fontWeight: 300, color: "#F5F0E8", lineHeight: 1.0, letterSpacing: "-0.02em", marginBottom: "3rem" }}>
            Six ways to
            <br />
            <em style={{ color: "#C9A84C" }}>elevate your space.</em>
          </h2>
        </motion.div>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {PATIO_FEATURES.map((feat, i) => <FeatureCard key={feat.title} feat={feat} p={p} i={i} />)}
        </div>
      </div>
    </motion.div>
  );
  ...
  return (
    <motion.div style={{ opacity }} className="absolute inset-0 z-10 flex items-center justify-center px-6">
      <motion.div style={{ y }} className="text-center max-w-3xl">
        <span className="text-xs tracking-[0.3em] uppercase block mb-8" style={{ color: "#C9A84C", fontSize: "10px" }}>The Investment</span>
        <blockquote style={{ fontFamily: "var(--font-display)", fontSize: "clamp(2.2rem, 5.5vw, 5.5rem)", fontWeight: 300, color: "#F5F0E8", lineHeight: 1.1, letterSpacing: "-0.02em", fontStyle: "italic" }}>
          ...
          <br />
          One built wrong costs twice.&rdquo;
        </blockquote>
  ...
  return (
    <motion.div style={{ opacity }} className="absolute inset-0 z-10 flex flex-col items-center justify-center px-6">
      <motion.div style={{ y }} className="text-center">
        <span className="text-xs tracking-[0.3em] uppercase block mb-8" style={{ color: "#C9A84C", fontSize: "10px" }}>Begin Your Project</span>
        ...
        <p style={{ color: "#8A8A8A", fontSize: "1rem", fontWeight: 300, maxWidth: "380px", margin: "2rem auto 2.5rem", lineHeight: 1.7 }}>
          Free 3D design consultation. We serve Ashburn, McLean, Loudoun County,
          Fairfax, Herndon, Sterling, and all of Northern Virginia.
        </p>
        <motion.button
        ...
        >
          Get My Free Patio Consultation
          <ArrowRight size={15} />
        </motion.button>
      </motion.div>
    </motion.div>
      ...
      <section className="py-28 px-6 md:px-20 max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.9 }}>
            <span className="text-xs tracking-[0.3em] uppercase block mb-6" style={{ color: "#C9A84C", fontSize: "10px" }}>Why Blue Lightning</span>
            ...
            <p style={{ color: "#8A8A8A", fontWeight: 300, lineHeight: 1.8, marginBottom: "2rem" }}>
              Virginia&apos;s freeze-thaw cycles destroy cheap patio installations within years.
              We engineer every base, drainage system, and paver installation for the local climate —
              so your investment looks as good in year 20 as it does on day one.
            </p>
            <div className="space-y-4">
              {[
                "Proper base preparation for Virginia freeze-thaw cycles",
                "Techo-Bloc & Unilock certified installation",
                "All grades and drainage engineered correctly",
                "Permits and HOA approvals handled for you",
                "Full 3D design before any material is ordered",
              ].map((item) => (
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.9, delay: 0.2 }}>
            <div className="relative rounded-3xl overflow-hidden" style={{ aspectRatio: "4/5" }}>
              ...
              <div className="absolute bottom-6 left-6 right-6 p-5 rounded-2xl" style={{ background: "rgba(13,13,13,0.85)", backdropFilter: "blur(16px)", border: "1px solid rgba(201,168,76,0.2)" }}>
                ...
                <div style={{ fontFamily: "var(--font-display)", fontSize: "1.4rem", color: "#F5F0E8", fontWeight: 300 }}>The Loudoun Estate</div>
                <div style={{ color: "#8A8A8A", fontSize: "12px", marginTop: "4px" }}>Loudoun County, VA · 2,200 sq ft · Techo-Bloc Pavers + Fire Pit</div>
              </div>
            </div>
          </motion.div>
        </div>
